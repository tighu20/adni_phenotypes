...
ukbb_data_summary <- ukbb_data %>%
  ...
  summarise(n = length(eid),
            ...
            sex = paste(length(sex[sex=="0"]),
                           length(sex[sex =="1"]),
                           sep = "/"),
            handedness = paste(length(handedness[handedness == 1]),
                               length(handedness[handedness == 2]),
                               length(handedness[handedness == 3]),
                               sep = "/"),
            .groups = "keep")
...
ukbb_data_summary <- ukbb_data %>%
  ...
  summarise(n = length(eid),
            ...
            sex = paste(round((length(sex[sex=="0"]) / length(eid)) * 100, digits = 2),
                        round((length(sex[sex =="1"]) / length(eid)) * 100, digits = 2),
                        sep = "/"),
            handedness = paste(round((length(handedness[handedness == 1]) / length(eid)) * 100, digits = 2),
                               round((length(handedness[handedness == 2]) / length(eid)) * 100, digits = 2),
                               round((length(handedness[handedness == 3]) / length(eid)) * 100, digits = 2),
                               sep = "/"),
            .groups = "keep")
