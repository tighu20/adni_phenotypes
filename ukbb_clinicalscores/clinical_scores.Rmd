...
alldata <- getdata(clinscore = clinscore,
  ...
  alldata$adlike <- as.factor(alldata$adlike)
  alldata$clinscore <- as.numeric(alldata$clinscore) + 1 # add one to permit ordinal regression model
  alldata$age <- as.numeric(alldata$age)
...
```{r, fig.height=2.5, fig.width=6}
cog_brms_draws <- spread_draws(cog_brms, b_adlikePositive)
...
#mean_intercept = mean(cog_brms_draws$b_Intercept)
sd_control = sd(alldata[alldata$adlike == "Negative", "clinscore"]) * 0.1
median_control = median(alldata[alldata$adlike == "Negative", "clinscore"]) * 0.1

cog_brms_draws %>%
#  dplyr::select(b_adlikePositive, b_Intercept) %>%
#  mutate(b_adlikePositive = median_control + b_adlikePositive) %>%
#  pivot_longer(cols = c(b_adlikePositive, b_Intercept)) %>%
#  group_by(name) %>%
  median_qi(.width = c(.8, .95)) %>%
  ggplot(aes(x = b_adlikePositive, xmin = .lower, xmax = .upper)) +
  geom_rect(xmin = median_control - sd_control,
            xmax = median_control + sd_control,
            ymin = 0, ymax = 3,
            fill = "cornsilk3") +
  geom_pointinterval() +
  geom_vline(xintercept = median_control, linetype = "dashed", colour = "blue") +
  geom_vline(xintercept = median_control + sd_control, linetype = "dashed", colour = "blue") +
  geom_vline(xintercept = median_control - sd_control, linetype = "dashed", colour = "blue") +
  scale_y_discrete(labels = c("AD-like positive")) +
  labs(x = clinscore_name) +
  theme(axis.title.y = element_blank(),
        axis.text = element_text(size = 15),
        axis.title.x = element_text(size = 15))
