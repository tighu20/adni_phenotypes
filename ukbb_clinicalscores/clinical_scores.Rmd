...
```{r}
# Bayesian analysis
priors <- c(prior(cauchy(3, 10), class = "Intercept"), 
...
```{r}
alldata %>%
  group_by(clinscore, adlike) %>%
  summarise(percent = length(eid)) %>%
  summarise(percent = percent/sum(percent)) %>%
  ggplot(aes(x = clinscore,
             y = count,
             fill = adlike)) +
