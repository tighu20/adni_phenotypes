...
alldata_summary <- alldata_summary %>%
  mutate(clinscore = as.factor(clinscore)) %>%
  group_by(adlike, clinscore) %>%
...
# Bayesian analysis - ordinal regression model
cog_brms <- brm(clinscore ~ adlike + age, 
                data = alldata, 
                iter = 5000, warmup = 1000, chains = 4,
                family = sratio())
...
cog_brms %>%
  ...
  geom_vline(aes(xintercept = mean(b_adlikePositive)),
             colour = "blue",
             linetype = "dashed") +
  geom_vline(xintercept = summary_model$fixed[3,3],
             colour = "blue",
             linetype = "dashed") + 
  geom_vline(xintercept = summary_model$fixed[3,4],
             colour = "blue",
             linetype = "dashed")
...
cog_brms_draws <- spread_draws(cog_brms, b_adlikePositive, b_Intercept[])
# select the third intercept
cog_brms_draws <- cog_brms_draws[seq(nrow(cog_brms_draws) / 2) * 2 - 1,] 
